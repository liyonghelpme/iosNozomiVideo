...
function BuildView:getVertexZ()
    local x, y = self.view:getPosition()
    if not self.state.moveOk then
        return 1001
    else
        y = y + self.scene.mapGrid.sizeY*self.buildMould.buildData.gridSize/2
    end
    return (y/self.scene.SIZEY)*-999+1000
end
--[[
function BuildView:setVertexZRecursive(obj, vz)
    local grids = obj:getChildren() 
    if grids ~= nil then
        for i = 0, grids:count()-1 do
            local child = grids:objectAtIndex(i)
            CCNode.setVertexZ(child, vertexZ)
        end
    end
end
--]]
function BuildView:setVertexZ()
    if true then
        return
    end
    local vertexZ = self:getVertexZ()
    if self.view ~= nil then
        
    end
    if self.debug then
        if self.depthInfo ~= nil then
            self.depthInfo:removeFromParentAndCleanup(true)
            self.depthInfo = nil
        end
        local bz = self.build:getVertexZ()
        local zo = self.view:getZOrder()

        self.depthInfo = CCLabelTTF:create(string.format("%.1f  %.1f %d ", vertexZ, bz, zo), "Arial", 20)
        self.view:addChild(self.depthInfo)
        self.depthInfo:setVertexZ(1000)
    end

    if self.build ~= nil then
        self.build:setShaderProgram(CCShaderCache:sharedShaderCache():programForKey("ShaderPositionTextureColorAlphaTest"))
        self.build:setVertexZ(vertexZ)

        local grids = self.build:getChildren() 
        if grids ~= nil then
            for i = 0, grids:count()-1 do
                local child = grids:objectAtIndex(i)
                CCNode.setVertexZ(child, vertexZ)
    ...
    if self.bottom ~= nil then
        local grids = self.bottom:getChildren() 
        if grids ~= nil then
            for i = 0, grids:count()-1 do
                local child = grids:objectAtIndex(i)
                CCNode.setVertexZ(child, vertexZ)
    ...
    if self.pt ~= nil then
        local grids = self.pt:getChildren() 
        if grids ~= nil then
            for i = 0, grids:count()-1 do
                local child = grids:objectAtIndex(i)
                CCNode.setVertexZ(child, vertexZ)
    ...
    if self.notice ~= nil then
        local grids = self.notice:getChildren() 
        if grids ~= nil then
            for i = 0, grids:count()-1 do
                local child = grids:objectAtIndex(i)
                CCNode.setVertexZ(child, vertexZ)
...
function BuildView:movePosition(position)
    ...
    --[[
    if self.buildMould.needAdjustBuild then
        self.build:setPosition(position[1], position[2])
    end
    --]]
...
--¿¿¿ ¿¿¿¿¿¿¿tempBuildId 
--¿¿¿¿¿¿¿ ¿¿¿¿¿ tempBuildId 
--bidToBuild ¿¿ tempBuildId ¿¿tempBuildId ¿¿¿ ¿¿¿¿¿¿¿¿¿
...
--[[
	local isTouch = false
	local scene = self.scene
	if scene.buyingBuild and not state.isBuying or self.deleted then
		return false
	end
...
function BuildView:addDestroyView()
	...
	if gridSize==5 then
		...
		if bid>=3000 and bid<=3007 and bid~=3006 then
...
function BuildView:onTouches(eventType, x, y)
...
function BuildView:update(diff)
	...
	--[[
	if state.movable then
		if state.touchPoint and not state.isFocus and state.touchTime and timer.getTime() - state.touchTime>0.5 then
			self:setFocus(true)
		end
	end
	--]]
	...
	if state.nightMode ~= UserData.isNight then
		...
		if self.build then
			if state.nightMode then
        	    ...
        	    if self.buildingView then
        	        self.buildingView:setColor(General.normalColor)
        	    end
			end
	        if self.buildMould.buildLevel>0 and self.buildMould.buildState==BuildStates.STATE_FREE then
			    self.buildMould:changeNightMode(state.nightMode)
			end
		end
	end
	...
	if sceneType==SceneTypes.Operation and not state.isBuying then
		...
		if self.buildMould then
    		...
    		if self.hitpoints<self.buildMould.buildData.hitPoints then
    		    self.hitpoints = squeeze(self.hitpoints+getParam("buildHitpointsSpeed", 50)*diff, self.hitpoints, self.buildMould.buildData.hitPoints)
    		    self.blood:changeValue(self.hitpoints)
    		    if self.isOperationDestroy and self.hitpoints > self.buildMould.buildData.hitPoints/5 then
...
function BuildView:ctor(buildMould, scene, setting)
    ...
    --¿¿¿¿¿¿ tempBuildId ¿¿¿¿ ¿¿buildMould ¿id ¿¿¿¿¿¿
    --if self.buildMould.id then
	...
	if (scene.sceneType==SceneTypes.Operation or scene.sceneType==SceneTypes.Visit) and params.hitpoints then
	    self.hitpoints = params.hitpoints
	...
	self.eventEntries = {update={inteval = inteval, callback=self.update}}
	if scene.sceneType == SceneTypes.Operation or scene.sceneType==SceneTypes.Zombie then
