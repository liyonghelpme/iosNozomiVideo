Decor={}
do
	Decor.addBuildFunctions = function(retBuild)
		...
		retBuild.createInfoMenu = function()
			...
			temp = UI.createTTFLabel(StringManager.getString("decorInfoStr"), General.defaultFont, 24, {colorR = 255, colorG = 255, colorB = 255})
			screen.autoSuitable(temp, {x=38, y=33, nodeAnchor=General.anchorLeft})
			bg:addChild(temp)
			local width = temp:getContentSize().width
			temp = UI.createTTFLabel(retBuild.getGoods().desc, General.defaultFont, 24, {colorR = 224, colorG = 194, colorB = 45})
