	...
	if self.toScene.sceneType==SceneTypes.Battle then
        for i, build in pairs(self.toScene.builds) do
            if build.setResourceState and build.resourceMax then
                print("preload", build.id, build.buildData.bid, build.resourceType,build)
...
function PreBattleScene:updateOthers(diff)
    if self.loadState==7 and not self.toScene.initInfo and not self.showPrepareView then
    	...
    	temp = UI.createMenuButton(CCSizeMake(111, 109), "images/buttonChildMenu.png", self.returnHome, self, "images/menuItemAchieve.png", StringManager.getString("buttonReturnHome"), MENU_BUTTON_SIZE)
    	...
    	showPrepareView:addChild(temp)
    elseif self.showPrepareView and self.loadState>7 then
